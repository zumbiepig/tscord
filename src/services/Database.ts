...
import { backupDatabase, dayjsTimezone, getFolderSize, isSQLiteDatabase, restoreDatabase } from '@/utils/functions';
...
export class Database {
	...
	async init() {
		...
		this.mikroORMConfig = defineConfig({
			entities: [...Object.values(entities), ...this.pluginsManager.getEntities()],
		...
		if (!this.store.get('botHasBeenReloaded')) {
			...
			const executedMigrations = await this._orm.migrator.getExecutedMigrations();
	...
	async backupDb(snapshotFile?: string): Promise<boolean> {
		...
		if (!databaseConfig.path) {
			await this.logger.log('error', "Database path not set, couldn't backup database");
			return false;
		}
	...
	async restoreDb(snapshotFile: string): Promise<boolean> {
		if (!isSQLiteDatabase()) {
			await this.logger.log('error', "Database is not SQLite, couldn't restore");
			return false;
		}

		if (!databaseConfig.path) {
			await this.logger.log('error', "Database path not set, couldn't restore backup");
			return false;
		}

		await stat(join(databaseConfig.path, 'backups', snapshotFile)).catch(async () => {
			await this.logger.log('error', `Snapshot ${snapshotFile} does not exist, couldn't restore backup`);
			return false;
		});
	...
	async getBackupList() {
		if (!databaseConfig.path) {
			await this.logger.log('error', "Database path not set, couldn't get list of backups");
			return;
		}
	...
	async getSize() {
		...
		return {
			db: isSQLiteDatabase() && mikroORMConfig.dbName ? (await stat(mikroORMConfig.dbName)).size : undefined,
			backups: await getFolderSize(backupPath).catch((error: unknown) => {
