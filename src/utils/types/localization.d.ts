...
type Sanitization<T extends unknown[]> = {
	[K in keyof T]: T[K] extends object ?
		...
		> extends true ?
			(Omit<T[K], (('name' | 'nameLocalizations') extends keyof T[K] ? ('name' | 'nameLocalizations') : never) | (('description' | 'descriptionLocalizations') extends keyof T[K] ? ('description' | 'descriptionLocalizations') : never)> &
			Simplify<
				(('name' | 'nameLocalizations') extends keyof T[K] ?
					('name' extends RequiredKeysOf<T[K]> ? { nameLocalizations: TranslationPaths }
						: 'name' extends OptionalKeysOf<T[K]> ? { nameLocalizations?: TranslationPaths }
							: never)
					: unknown) &
				(('description' | 'descriptionLocalizations') extends keyof T[K] ?
					('description' extends RequiredKeysOf<T[K]> ? { descriptionLocalizations: TranslationPaths }
						: 'description' extends OptionalKeysOf<T[K]> ? { descriptionLocalizations?: TranslationPaths }
							: never)
					: unknown)
			>)
		: T[K])
...
type testa = Sanitization<[any,unknown,never,{},{name: string, nameLocalizations: string, description?: string, descriptionLocalizations?: string,shouldConvert: boolean},...args]>

export type SlashGroupOptions<T extends string, TD extends string> = Sanitization<Parameters<typeof SlashGroupX<T,TD>>>;
