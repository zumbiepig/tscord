...
export const DisplayCard: React.FC<DisplayCardProps> = ({ image, title, children, ...rest }) => {
	return (
		<>
			<Card py="7px" role="group" position="relative" px={{ base: 5, sm: 6 }} w="100%" h="100%" {...rest}>
				<HStack spacing="2em" position="relative">
					{typeof image === 'string' ? <Image src={image} alt={title} w="60px" h="60px" borderRadius="50%" /> : image}
