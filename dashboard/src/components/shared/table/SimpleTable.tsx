...
interface Props {
	...
	tableData: object[];
	cellsResolvers?: Record<string, (cell: Cell<object>) => React.ReactNode>;
	additionalProps?: any;
}
						...
						{table.headerGroups.map((headerGroup, index) => (
							<Tr {...headerGroup.getHeaderGroupProps()} key={index}>
								{headerGroup.headers.map((column, index) => (
									<Th {...column.getHeaderProps(column.getHeaderProps())} key={index} borderColor={borderColor}>
							...
							return (
								<Tr {...row.getRowProps()} key={index}>
									{row.cells.map((cell, index) => (
										...
										>
											{cellsResolvers ? cellsResolvers[cell.column.id](cell) : cell.render('Cell')}
										</Td>
									))}
								</Tr>
							);
