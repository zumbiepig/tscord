...
interface AvailableBotsProps {
	userId: string;
}

const processBotList = (bots: BotsState) => {
	return bots.authorized
		.map((bot) => ({ ...bot, state: 'authorized' }))
		.concat(bots.offline.map((bot) => ({ ...bot, state: 'offline' })));
};

export const AvailableBots: React.FC<AvailableBotsProps> = ({ userId }) => {
	const bots = useSWR<BotsState>('/allUserAuthorizations', (url: string) =>
		fetcher(url, undefined, { userId: userId }),
	);

	return (
		<>
			<Heading as="h2" mt="2em" mb="1em">
				Bots
			</Heading>
