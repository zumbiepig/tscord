...
export const DetailedGuild: React.FC<DetailedGuildProps> = ({
  isOpen,
  onClose,
  guild,
}) => {
  ...
  const { name, iconURL, memberCount, description, preferredLocale } =
    guild.discord;
      ...
      .then((res) => {
        if (res.status === 200)
          successToast(
            toast,
            'Guild deleted',
            `Guild "${guild.discord.name}" has been deleted`,
          );
        else
          errorToast(
            toast,
            'Error',
            `Error deleting guild "${guild.discord.name}"`,
          );
    ...
    <>
      <Modal isOpen={isOpen} onClose={onClose} isCentered>
        ...
        <ModalContent
          position="relative"
          minH="500px"
          minW={['unset', 'unset', 'unset', '800px', '800px', '800px']}
          w={['90vw', '90vw', '90w', '50vw', '50vw', '50vw']}
        >
          <ModalHeader>
            <Stack
          ...
          <ModalBody>
            <VStack align="start" spacing="1em">
              <TextSection title="Description" text={description ?? 'N/A'} />

              <Wrap spacing="5em">
                <TextSection title="Last Activity">
                  <Tag
                    bg={getActivityColor(lastInteract).replace(
                      '500',
                      useColorModeValue('100', '900'),
                    )}
                  >
                    <Circle
                      size="12px"
                    ...
                    <Text>
                      {lastInteract
                        ? timeAgo.format(new Date(lastInteract).getTime())
                        : 'N/A'}
                    </Text>
                  </Tag>
                </TextSection>
