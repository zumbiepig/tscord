...
const Logo: React.FC = () => {
	...
	return (
		<Flex align="center" direction="column">
			<Menu>
				<Flex w="100%" px={10} mb="1em" direction="row" justify="space-between" align="center">
			...
			{tabs.map((tab, index) => (
				<Link as={NextLink} href={`/admin/${currentBot.id}/${tab.href}`} key={index}>
					<Box _hover={{ cursor: 'pointer' }}>
						<HStack spacing={isActiveTab(tab.href) ? '22px' : '26px'} py="5px" ps="10px">
							...
							<Box h="36px" w="4px" bg={isActiveTab(tab.href) ? brandColor : 'transparent'} borderRadius="5px" />
						</HStack>
					</Box>
				</Link>
			))}
