...
const StatisticsPage: NextPage<AdminDashboardProps> = ({
  bots,
  authorizedBots,
  currentBot,
}) => {
  const stats = {
    ...
    topCommands: useSWR('/stats/commands/top', (url) =>
      fetcher(url, currentBot.id),
    ),
    topGuilds: useSWR('/stats/guilds/top', (url) =>
      fetcher(url, currentBot.id),
    ),
    lastInteraction: useSWR('/stats/interaction/last', (url) =>
      fetcher(url, currentBot.id),
    ),
    lastGuildAdded: useSWR('/stats/guilds/last', (url) =>
      fetcher(url, currentBot.id),
    ),
    commandsUsage: useSWR('/stats/commands/usage', (url) =>
      fetcher(url, currentBot.id, { numberOfDays: 7 }),
    ),
    usersActivity: useSWR('/stats/users/activity', (url) =>
      fetcher(url, currentBot.id),
    ),
    usersAndGuilds: useSWR('/stats/usersAndGuilds', (url) =>
      fetcher(url, currentBot.id, { numberOfDays: 7 }),
    ),
  };
  ...
  const commandsUsageSeries = [
    {
      ...
      data:
        stats.commandsUsage.data
          ?.map(
            (commandUsage: { [key: string]: any }) =>
              commandUsage.simpleCommands,
          )
          .reverse() || [],
    },
    {
      ...
      data:
        stats.commandsUsage.data
          ?.map(
            (commandUsage: { [key: string]: any }) => commandUsage.contextMenus,
          )
          .reverse() || [],
    },
    {
  ...
  const usersActivity = {
    values: stats.usersActivity.data
      ? Object.values(stats.usersActivity.data)
      : [0],
    labels: stats.usersActivity.data
      ? Object.keys(stats.usersActivity.data)
      : [],
  };
      ...
      >
        ...
        >
          ...
          <StatCard
            ...
            value={
              stats.lastInteraction.data?.createdAt
                ? `${timeAgo.format(new Date(stats.lastInteraction.data.createdAt).getTime(), 'mini')} ago`
                : ''
            }
            icon={<BiTimeFive />}
          />
          <StatCard
            ...
            value={
              stats.lastGuildAdded.data?.createdAt
                ? `${timeAgo.format(new Date(stats.lastGuildAdded.data.createdAt).getTime(), 'mini')} ago`
                : ''
            }
            icon={<BiTimeFive />}
          />
                  ...
                  <BarChart
                    ...
                    options={{
                      xaxis: {
                        categories:
                          stats.commandsUsage.data
                            ?.map((commandUsage: { [key: string]: any }) =>
                              commandUsage.date
                                .split('/')
                                .slice(0, -1)
                                .join('/'),
          ...
          {stats.topCommands.data && (
            <>
              <SimpleTable
                ...
                tableData={stats.topCommands.data.map(
                  (command: { name: string; type: string; count: number }) => ({
                    name: command.name,
                    type: typeResolver[
                      command.type as keyof typeof typeResolver
                    ],
                    count: command.count,
                  }),
                )}
              ...
              series={[
                {
                  name: 'Guilds',
                  data:
                    stats.usersAndGuilds.data?.guilds
                      .map((guild: any) => guild.count)
                      .reverse() || [],
                },
                {
                  name: 'Users',
                  data:
                    stats.usersAndGuilds.data?.users
                      .map((user: any) => user.count)
                      .reverse() || [],
                },
              ]}
              options={{
                xaxis: {
                  categories:
                    stats.usersAndGuilds.data?.users
                      .map((user: any) =>
                        user.date.split('/').slice(0, -1).join('/'),
                      )
                      .reverse() || [],
                },
              }}
