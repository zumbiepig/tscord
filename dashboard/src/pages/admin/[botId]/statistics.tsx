...
import { BarChart, ChartCard, LineChart, PieChart, SimpleSwitcher, SimpleTable, StatCard } from '@components/shared';
...
const StatisticsPage: NextPage<AdminDashboardProps> = ({ bots, authorizedBots, currentBot }) => {
	...
	const commandsUsageSeries = [
		...
		},
		{
			name: 'Slash commands',
			data:
				stats.commandsUsage.data?.map((commandUsage: Record<string, any>) => commandUsage.slashCommands).reverse() ||
				[],
		},
	];
		...
		<>
			<AdminDashboard breadcrumbs={['Statistics']} bots={bots} authorizedBots={authorizedBots} currentBot={currentBot}>
				<SimpleGrid columns={{ base: 2, md: 2, lg: 3, '2xl': 6 }} gap="20px" mb="20px">
					<StatCard title="Total commands" value={stats.totals.data?.stats.totalCommands} icon={<HiOutlineCode />} />
					<StatCard title="Total guilds" value={stats.totals.data?.stats.totalGuilds} icon={<SiClubhouse />} />
					<StatCard title="Total Users" value={stats.totals.data?.stats.totalUsers} icon={<FaUserFriends />} />
					<StatCard
				...
				<SimpleGrid columns={{ base: 1, md: 1, xl: 2 }} gap="20px" mb="20px">
											...
											xaxis: {
												categories:
													stats.commandsUsage.data
														?.map((commandUsage: Record<string, any>) =>
															commandUsage.date.split('/').slice(0, -1).join('/'),
														)
														.reverse() || [],
											},
										}}
					...
					<SimpleGrid columns={{ base: 1, md: 1, xl: 2 }} gap="20px" mb="20px">
						<ChartCard title="Users" subtitle="Part of active users of the bot inside all the known users">
							<PieChart
								...
								xaxis: {
									categories:
										stats.usersAndGuilds.data?.users
											.map((user: any) => user.date.split('/').slice(0, -1).join('/'))
											.reverse() || [],
								},
							}}
						/>
