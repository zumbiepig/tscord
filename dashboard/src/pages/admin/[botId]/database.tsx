...
const DatabasePage: NextPage<AdminDashboardProps> = ({
  bots,
  authorizedBots,
  currentBot,
}) => {
  ...
  const backupList = useSWR('/database/backups', (url) =>
    fetcher(url, currentBot.id),
  );
  const databaseSize = useSWR('/database/size', (url) =>
    fetcher(url, currentBot.id),
  );
            ...
            <Button
              onClick={() => {
                fetch(`/api/bot/${currentBot.id}/database/restore`, {
                  ...
                  .then((res) => {
                    if (res.status === 200)
                      successToast(toast, 'Backup restored successfuly');
  ...
  return (
    <>
      ...
      >
        <SimpleGrid columns={[1, 1, 1, 1, 1, 3, 3]} gap="20px" mb="20px">
          <Flex flexDirection="column">
            ...
            <SimpleGrid columns={[1, 2, 1, 1, 1, 2]} gap="20px" my="20px">
              <StatCard
                title="Database size"
                value={
                  databaseSize.data?.db
                    ? `${(databaseSize.data.db / 1_000_000).toFixed(2)} Mo`
                    : '-'
                }
              ...
              <StatCard
                title="Backups size"
                value={
                  databaseSize.data?.backups
                    ? `${(databaseSize.data.backups / 1_000_000).toFixed(2)} Mo`
                    : '-'
                }
          ...
          <Box gridColumn={{ md: '2', lg: '2 / 4' }} h="500px">
            {backupList.data && (
              <SimpleTable
                title="Backups"
                columnsData={tables.backupList.columns}
                tableData={backupList.data
                  .map((backup: string) => ({ name: backup, restore: backup }))
                  .reverse()}
