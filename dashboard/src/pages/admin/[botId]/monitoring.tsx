...
const MonitoringPage: NextPage<AdminDashboardProps> = ({ bots, authorizedBots, currentBot }) => {
	...
	const maintenance = useSWR<{ maintenance: boolean }>('/bot/maintenance', (url) => fetcher(url, currentBot.id), {
		refreshInterval,
	});
	const monitoring = useSWR<MonitoringData[]>('/health/monitoring', (url) => fetcher(url, currentBot.id), {
		refreshInterval,
	});
	...
	const getLatestData = () => monitoringDataHistory.at(-1);
			...
			>
				{monitoring.error ? (
					...
					<>
								...
								<Card>
									...
									<Flex direction="column" align="center" ml="2em">
										...
										<Text
											fontSize="2xl"
											fontWeight="bold"
											color={getLatestData()?.botStatus.online ? 'green.500' : 'red.500'}
										>
								...
								>
									...
									<Flex direction="column" align="center" ml="2em">
										...
										<Text
											fontSize="2xl"
											fontWeight="bold"
											color={maintenance.data?.maintenance ? 'green.500' : 'gray.500'}
										>
						...
						<AlertDialog isOpen={isConfirmOpen} leastDestructiveRef={cancelConfirmRef} onClose={onConfirmClose}>
							<AlertDialogOverlay>
								<AlertDialogContent>
									...
									<AlertDialogBody>
										Are you sure to <em>{getLatestData()?.botStatus.maintenance ? 'disable' : 'enable'}</em> maintenance
										mode?
									</AlertDialogBody>
