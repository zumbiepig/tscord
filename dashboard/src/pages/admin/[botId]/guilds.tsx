...
const GuildsPage: NextPage<AdminDashboardProps> = ({
  bots,
  authorizedBots,
  currentBot,
}) => {
  ...
  const processedGuildsData = guildsData
    ? guildsData
        .filter((guild: any) => {
          return search
            ? guild.discord.name.toLowerCase().includes(search.toLowerCase())
            : true;
        })
        .sort((a: any, b: any) => {
          if (sortBy === 'name') {
            return a.discord.name
              .toLowerCase()
              .localeCompare(b.discord.name.toLowerCase());
          } else if (sortBy === 'members') {
            return b.discord.memberCount - a.discord.memberCount;
          } else if (sortBy === 'activity') {
            return (
              new Date(b.database.lastInteract).getTime() -
              new Date(a.database.lastInteract).getTime()
            );
          }
        })
    : [];
